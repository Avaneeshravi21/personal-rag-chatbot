...
# Streamlit Cloud stores secrets in st.secrets, not as regular OS
# environment variables -- but config.py (shared with the local CLI/API
# versions) reads keys via os.getenv, which only sees real env vars.
# This bridges the two: copy every Streamlit secret into the environment
# BEFORE anything else imports config.py, so the exact same config code
# works unchanged whether running locally (.env file) or on Streamlit
# Cloud (secrets box).
